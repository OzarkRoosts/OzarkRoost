...
function softStart(label, fn) {
  try { fn(); } catch (err) { console.warn(`[startup] ${label} failed soft-start:`, err.message); }
}
...
async function startServer() {
  ...
  softStart('site-health', () => { if (siteHealth) siteHealth.start(); });

  const affiliateOps = softRequire('./lib/affiliate-ops-agent');
  softStart('affiliate-ops', () => { if (affiliateOps) affiliateOps.start(); });
  ...
  const partnerDiscovery = softRequire('./lib/partner-discovery-agent');
  softStart('partner-discovery', () => {
    if (partnerDiscovery && process.env.PARTNER_DISCOVERY_ENABLED !== 'false') partnerDiscovery.start();
  });
  ...
  softStart('marketing', () => {
    ...
    if (marketing?.start) { marketing.start(); console.log('[Marketing] engine armed.'); }
  });
  ...
  app.get('/sitemap.xml', (req, res) => {
    ...
    const body = ['<?xml version="1.0" encoding="UTF-8"?>', '<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">', ...SITEMAP_PATHS.map((p) => ['  <url>', `    <loc>${base}${p}</loc>`, '    <changefreq>weekly</changefreq>', `    <priority>${p === '/' ? '1.0' : p.startsWith('/guides/') ? '0.9' : '0.7'}</priority>`, '  </url>'].join('\n')), '</urlset>', ''].join('\n');
  ...
  app.get('/superagent-status', (_req, res) => {
    try { const superagent = require('./lib/super-agent'); res.json(superagent.getStatus()); }
    catch (err) { res.status(503).json({ error: 'Super Agent not enabled', detail: err.message }); }
  });
  ...
  app.get('/listings', async (_req, res) => {
    ...
    res.render('listings', { listings: await getAllListings() });
  });

  app.get('/adventures', (_req, res) => {
    const { getAffiliateLinks, getFeaturedListings } = require('./lib/affiliate-links');
    res.render('adventures', { affiliateLinks: getAffiliateLinks(), featuredListings: getFeaturedListings() });
  });
  ...
  function isAllowedPartnerUrl(raw) {
    ...
    try { const u = new URL(raw); return u.protocol === 'https:' && PARTNER_HOST_ALLOW.has(u.hostname.toLowerCase()); }
    catch { return false; }
  }
  ...
  app.get('/out', outLimiter, async (req, res) => {
    ...
    if (toRaw && !listingId) {
      let target = toRaw;
      try { target = decodeURIComponent(toRaw); } catch {}
      if (!isAllowedPartnerUrl(target)) return res.redirect('/guides/hot-tub-cabins');
      pool.query('INSERT INTO affiliate_clicks (listing_id, partner, user_agent, clicked_at) VALUES ($1, $2, $3, NOW())', [null, partner || 'direct', sanitizeText(req.headers['user-agent'], 300) || null]).catch(() => {});
      return res.redirect(302, target);
    }
    if (!listingId || !partner || !/^\d+$/.test(listingId)) return res.redirect('/listings');
    try {
      const row = await pool.query('SELECT website_url FROM listing_submissions WHERE id = $1 AND payment_status = $2', [listingId, 'paid']);
      if (!row.rows[0]) return res.redirect('/listings');
      const target = row.rows[0].website_url;
      if (!isSafeExternalUrl(target)) return res.redirect('/listings');
      pool.query('INSERT INTO affiliate_clicks (listing_id, partner, user_agent, clicked_at) VALUES ($1, $2, $3, NOW())', [listingId, partner, sanitizeText(req.headers['user-agent'], 300) || null]).catch(err => console.error('[affiliate_clicks] insert error:', err?.message));
      res.redirect(302, target);
    } catch (err) { console.error('[/out route] error:', err?.message); res.redirect('/listings'); }
  });
  ...
  app.use('/api/partner-discovery', apiLimiter, require('./routes/partner-discovery'));
