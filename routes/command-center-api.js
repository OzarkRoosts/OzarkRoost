...
const outreach = require('../lib/proactive-outreach');
...
router.use((req, res, next) => authorize(req, res) ? next() : undefined);
...
router.get('/summary', async (_req, res) => {
  try {
    ...
    res.json({
      ...
      worker: outreach.run ? { enabled: process.env.OPSBOT_PROACTIVE_OUTREACH === 'true' } : { enabled: false }
    });
  } catch (err) {
...
router.get('/outreach', async (_req, res) => {
  try {
