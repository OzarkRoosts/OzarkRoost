...
router.post('/send', async (req, res) => {
  try {
    ...
    await killerDb.logEmailEvent(campaign_id, 'sent', { recipient: to, timestamp: new Date() });
...
router.post('/contract', async (req, res) => {
  try {
    const { prospect_email, prospect_name, company, service, price, terms = '12 months, auto-renew', campaign_id } = req.body;
    if (!prospect_email || !prospect_name || !price) {
      return res.status(400).json({ error: 'Missing required: prospect_email, prospect_name, price' });
    }
    await autonomous.sendContract({
      prospect_email,
      prospect_name,
      company,
...
/**
 * Contract acceptance is intentionally converted into an explicit payment request.
 * No email reply or API request is allowed to create an off-session subscription.
 */
router.post('/accept-signature', async (req, res) => {
  try {
    ...
    if (!prospect_email || !price) {
      return res.status(400).json({ error: 'Missing required: prospect_email, price' });
    }
    ...
    res.status(202).json({
      ...
      message: 'No card was charged. An explicit Stripe payment link was sent to the customer.',
...
router.post('/charge', async (req, res) => {
  try {
    const { prospect_email, prospect_name, amount, description = 'Payment', type = 'one-time' } = req.body;
    if (!prospect_email || !amount) {
      return res.status(400).json({ error: 'Missing required: prospect_email, amount' });
    }
    const result = await autonomous.sendInvoice({
      prospect_email,
      prospect_name: prospect_name || 'Customer',
    ...
    res.json({ success: true, message: `Payment link sent for $${amount}`, paymentLink: result.paymentLink, type });
  } catch (err) {
...
router.post('/monitor', async (req, res) => {
  res.status(202).json({
    success: true,
    status: 'SAFE MONITORING MODE',
    message: 'Monitoring/response automation is available, but the legacy autonomous billing workflow is disabled until its simulated acceptance path is removed.',
    billing: 'CUSTOMER_ACTION_REQUIRED',
    tasks: [
      'Monitor for prospect replies',
      'Auto-respond intelligently',
      'Send contracts for signature',
      'Send explicit Stripe payment links',
...
router.get('/activate', async (req, res) => {
  res.status(202).json({
    ...
    message: 'Direct off-session charging from email acceptance is disabled.',
  });
});
