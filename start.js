...
async function start() {
  ...
  // Exactly one production owner for outbound local-business outreach:
  // site-health-agent -> proactive-outreach. Legacy facades are never scheduled.
  if (process.env.OUTREACH_AUTOMATION_ENABLED === 'false') {
    console.log('[Outreach] automated outbound disabled by OUTREACH_AUTOMATION_ENABLED=false');
  } else {
    console.log('[Outreach] canonical worker delegated to site-health startup');
  }
