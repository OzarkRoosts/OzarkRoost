...
function errorHandler() {
  return function handleError(err, req, res, next) {
    ...
    pushEvent({ type: 'express-error', method: req.method, path: req.originalUrl || req.url, message, stack, status: 500 });
...
function getStats() {
  ...
  return { total_requests: totalRequests, total_5xx: total5xx, unhandled, rate: Math.round(rate * 1000) / 1000, recent_5xx_15m: recent5xx, recent_events: recent.slice(0, 20).map((e) => ({ ...e, at: new Date(e.at).toISOString() })) };
}

function reset() { ring.length = 0; totalRequests = 0; total5xx = 0; unhandled = 0; }
