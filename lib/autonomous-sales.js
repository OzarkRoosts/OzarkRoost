...
const { shouldProcessContract } = require('./sales-billing-policy');
...
function getAIClient() {
  ...
  if (!apiKey) {
    const error = new Error('AI credentials are not configured');
...
async function aiReply({ prospect_name, company, subject }) {
  ...
  const response = await client.chat.completions.create({
    ...
    temperature: 0.35,
    messages: [
      { role: 'system', content: 'You are a concise, professional sales representative for OzarkRoost, an Arkansas Ozarks cabin and outdoor directory. Move interested prospects toward an OzarkRoost paid listing; use the current $99/$149/$199 plans when pricing is discussed. Never claim a payment was made unless Stripe confirms it. Always give one clear next step.' },
...
async function processContractAcceptance(args) {
  if (!shouldProcessContract(args?.status)) {
    throw new Error('explicit customer acceptance is required before billing');
  }
  return sendInvoice({
