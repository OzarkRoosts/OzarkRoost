...
function scoreGap({ page, detected, category, platform }) {
  ...
  if (page.widgets?.length && !page.widgets.some((w) => detected.widgets.includes(w))) priority += 1;
...
async function upsertOpportunity(opp) {
  try {
    const existing = await pool.query(
      `SELECT id, status FROM affiliate_opportunities
    ...
    const ins = await pool.query(
      ...
      [opp.page_path, opp.page_type, opp.opportunity_type, opp.platform, opp.suggested_text, opp.priority, opp.estimated_value]
    );
    return { id: ins.rows[0].id, status: ins.rows[0].status, created: true };
  } catch (err) {
    if (/relation .* does not exist/i.test(err.message || '')) return { id: null, status: null, created: false, skipped: true };
    throw err;
  }
}
...
/** Build a deterministic, deduplicated action queue from the gaps we already know about. */
function buildActionQueue(gaps, coverage) {
  ...
  for (const g of gaps) {
    ...
    queue.push({
      ...
      how: g.page_path.startsWith('/guides/')
        ? `Review views${g.page_path}.ejs and add/fix the affiliate widget for ${g.platform}.`
        : `Review ${g.page_path} and add a tracked ${g.platform} CTA.`,
    });
  }
  ...
  for (const [platform, info] of Object.entries(coverage)) {
    ...
    queue.push({
      ...
      how: `Set ${info.env_key} in Render/host env to the approved tracked partner link.`,
    });
  }

  queue.sort((a, b) => (b.priority || 0) - (a.priority || 0) || a.action_key.localeCompare(b.action_key));
  return queue.map((item, index) => ({ step: index + 1, ...item }));
}
...
function applySafePlan(plan) {
  ...
  plan.link_pack = {};
  for (const [key, url] of Object.entries(links)) plan.link_pack[key] = withUtm(url, '/ops', key);
...
async function runScan() {
  ...
  for (const page of PAGE_CATALOG) {
    ...
    const detected = page.path.startsWith('/guides/')
      ...
      : { hasWidgetPartial: true, widgets: page.widgets || [], hasFeatured: true, hasEmail: page.path === '/guides/trip-planner' };

    const pageGaps = [];
    for (const category of page.potential) {
      const platform = PLATFORM_FOR[category] || 'stay22';
      const hasPlatformWidget = detected.widgets.includes(platform) || (platform === 'stay22' && detected.widgets.includes('vrbo'));
      const weak =
      ...
      const opp = {
        ...
        priority,
        estimated_value,
        reason: !detected.hasWidgetPartial ? 'missing_affiliate_widget_partial' : !hasPlatformWidget ? 'missing_recommended_platform_widget' : 'missing_link_config',
      };
      pageGaps.push(opp);
      gaps.push(opp);
    ...
    const monScore = Math.max(0, 100 - pageGaps.length * 15 - (detected.hasWidgetPartial ? 0 : 20) - (detected.hasEmail || page.type !== 'guide' ? 0 : 10));
    await upsertPageStatus(page.path, page.type, detected, monScore);
    pageReports.push({ path: page.path, type: page.type, detected, gap_count: pageGaps.length, monetization_score: monScore, recommended_widgets: page.widgets });
  }

  gaps.sort((a, b) => b.priority - a.priority || b.estimated_value - a.estimated_value);
  const coverage = envCoverage();
  const actions = buildActionQueue(gaps, coverage);
  const pending_value = gaps.reduce((s, g) => s + (Number(g.estimated_value) || 0), 0);
  const human_actions = actions.filter((a) => a.mode === 'human_approval').length;
  const auto_actions = actions.filter((a) => a.mode === 'automatic').length;
  ...
  console.log(`[AffiliateOps] scan complete — gaps=${gaps.length} created=${created} actions_created=${actions.length} auto_actions=${auto_actions} human_actions=${human_actions} value~$${plan.totals.pending_value}`);
  return plan;
}

function getPlan() {
  return lastPlan || { agent: 'affiliate-ops', status: 'no_scan_yet', message: 'Call runScan() or wait for interval' };
}
...
function start(options = {}) {
  ...
  setTimeout(() => runScan().catch((err) => console.error('[AffiliateOps] scan error:', err.message)), options.initialDelayMs ?? 20_000);
  timer = setInterval(() => runScan().catch((err) => console.error('[AffiliateOps] scan error:', err.message)), SCAN_INTERVAL_MS);
...
module.exports = { start, stop, runScan, getPlan, getStatus, applySafePlan, PAGE_CATALOG, withUtm };
