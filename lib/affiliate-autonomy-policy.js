...
const PAYMENT_FIELD_RE = /(?:name|id)=[\"'][^\"']*(?:card|credit.?card|debit.?card|cvv|cvc|billing|payment.?method|routing.?number|bank.?account)[^\"']*[\"']/i;
const PAID_PLAN_RE = /(?:paid\s+(?:plan|subscription|membership)|subscription\s+(?:plan|fee)|monthly\s+fee|(?:premium|pro|business)\s+plan\s*[^<]{0,80}\$\s*\d|\$\s*\d[^<]{0,80}(?:\/month|\/mo|per\s+month))/i;
...
function isNoSpendApplication({ programName = '', html = '' } = {}) {
  ...
  return !PAYMENT_FIELD_RE.test(text) && !PAID_PLAN_RE.test(text);
}
