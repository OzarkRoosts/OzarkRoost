...
function classifyConversationGuard({ sender = '', subject = '', body = '' }) {
  if (isAutomatedInboundSender(sender, subject, body)) {
    return { status: 'ignored', category: 'automated_delivery' };
  }
  const text = `${subject}\n${body}`.trim();
  if (/\b(remove|delete|take down|unlist)\b[\s\S]{0,120}\b(us|our|business|listing|company)\b|\bdo not contact|\bstop (email|contacting|outreach)\b/i.test(text)) {
    return { status: 'ignored', category: 'contact_suppressed' };
  }
  if (/\b(false|incorrect|wrong|inaccurate|not true|misleading)\b[\s\S]{0,120}\b(info|information|listing|website|site|business|company)\b|\bwe (are not|don't|do not) want to be listed\b/i.test(text)) {
    return { status: 'review', category: 'business_complaint' };
  }
  if (/\b(paid|payment|charge|charged|invoice|receipt|billing)\b[\s\S]{0,100}\b(check|already|thought|did|can you|where|when)\b/i.test(text)) {
    return { status: 'review', category: 'payment_review' };
  }
  return { status: 'unread', category: null };
}

const GUARD_SQL = `
  CREATE OR REPLACE FUNCTION opsbot_guard_inbound_conversation()
  RETURNS trigger
  LANGUAGE plpgsql
  AS $$
  BEGIN
    ...
    THEN
      NEW.status := 'ignored'; NEW.category := 'automated_delivery'; RETURN NEW;
    END IF;
    IF lower(coalesce(NEW.subject, '') || ' ' || coalesce(NEW.body_text, '')) ~ '(remove|delete|take down|unlist).{0,120}(us|our|business|listing|company)|(do not contact)|(stop (email|contacting|outreach))' THEN
      NEW.status := 'ignored'; NEW.category := 'contact_suppressed'; RETURN NEW;
    END IF;
    IF lower(coalesce(NEW.subject, '') || ' ' || coalesce(NEW.body_text, '')) ~ '(false|incorrect|wrong|inaccurate|not true|misleading).{0,120}(info|information|listing|website|site|business|company)|(we are not|we don.t|we do not) want to be listed' THEN
      NEW.status := 'review'; NEW.category := 'business_complaint'; RETURN NEW;
    END IF;
    IF lower(coalesce(NEW.subject, '') || ' ' || coalesce(NEW.body_text, '')) ~ '(paid|payment|charge|charged|invoice|receipt|billing).{0,100}(check|already|thought|did|can you|where|when)' THEN
      NEW.status := 'review'; NEW.category := 'payment_review'; RETURN NEW;
    END IF;
    RETURN NEW;
  END;
  $$;
  ...
  FOR EACH ROW
  EXECUTE FUNCTION opsbot_guard_inbound_conversation();
