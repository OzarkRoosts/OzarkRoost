...
async function discoverCandidates({ kind, limit = 25 } = {}) {
  ...
  return result.rows.map(row => {
    ...
    return {
      ...
      integrationEffort: row.application_url ? 0.3 : 0.6,
      approvalLikelihood: row.application_url ? 0.75 : 0.5,
      ...
      automationPermitted: kind === 'sponsorship' ? Boolean(contactEmail) : metadata.automationPermitted === true,
      requiredFieldsAvailable: kind === 'sponsorship' ? Boolean(contactEmail) : metadata.requiredFieldsAvailable === true,
