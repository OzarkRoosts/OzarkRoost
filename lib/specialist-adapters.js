...
function wrap(agentId, capability, handler, invoke) {
  ...
  return async action => {
    ...
    return {
      result,
      evidence: evidence(agentId, capability, action.target, result),
...
function buildSpecialistAdapters(handlers = {}) {
  ...
  adapters['affiliate-ai:discover_affiliate'] = wrap(
    ...
    () => ai.runOpportunityScan(),
  );

  adapters['affiliate-ai:score_opportunity'] = wrap(
    'affiliate-ai', 'score_opportunity', ai?.runOpportunityScan,
    () => ai.runOpportunityScan(),
  );
  ...
  // OpsBot eagerly constructs its AI client, so keep it lazy: missing AI
  // credentials must block only the requested operation, not runtime startup.
  if (handlers.opsbot?.runPaymentWatchdog) {
    adapters['opsbot:monitor_ops'] = wrap(
      'opsbot', 'monitor_ops', handlers.opsbot.runPaymentWatchdog,
      action => handlers.opsbot.runPaymentWatchdog(action.payload || {}),
    );
  }

  adapters['rover:answer'] = wrap(
    'rover', 'answer', rover?.answer,
    action => rover.answer(action.payload?.message || action.target || ''),
  );
...
function loadDefaultSpecialistAdapters() {
  ...
  // Do not load OpsBot during runtime construction; its module creates an AI
  // client eagerly. The server can inject OpsBot when credentials are ready.
