...
function createSpecialistAdapters(handlers = {}) {
  ...
  for (const [capabilityKey, [handlerKey, method]] of Object.entries(HANDLER_MAP)) {
    ...
    adapters[capabilityKey] = async action => {
      let value;
      if (agentId === 'rover') value = await handler[method](action.payload?.message || action.payload || {});
      else if (['affiliate-ai', 'affiliate-executor', 'affiliate-ops', 'autonomous-sales'].includes(agentId)) value = await handler[method]();
      else value = await handler[method](action.payload || {});
