...
async function sendEmail({ to, subject, html, text }) {
  ...
  if (!from) {
    console.warn('[OpsBot] No outbound email sender configured — skipping send to', to);
    return null;
  }
  try {
...
async function runEmailSmokeTest() {
  ...
  if (!to) {
    console.warn('[OpsBot:EmailTest] skipped — no EMAIL_USER/SMTP_USER configured');
    return;
  }
  try {
  ...
  console.log('[OpsBot:EmailTest] Sending outbound SMTP smoke test to configured owner address...');
...
async function processInboundEmail(email) {
  try {
    const category = await aiChat(`You are an email classifier for OzarkRoost, a cabin and outdoor recreation directory in the Arkansas Ozarks.
...
Respond with only the category name.`, `From: ${email.sender}\nSubject: ${email.subject}\nBody: ${email.body_text?.substring(0, 500)}`);
    const reply = await aiChat(`You are OpsBot, the autonomous operations assistant for OzarkRoost.
...
Affiliate commissions: 8-10%. Always end with a clear next step.`, `Email category: ${category}\nFrom: ${email.sender}\nSubject: ${email.subject}\nBody: ${email.body_text?.substring(0, 800)}`);
...
async function runAffiliateHunter() {
  ...
  try {
    for (const target of AFFILIATE_TARGETS) {
      ...
      const appText = await aiChat(`You are writing an affiliate program application for OzarkRoost.
...
Write a professional 3-paragraph affiliate application email. Be genuine, mention our niche relevance.`, `Affiliate program: ${target.name}\nCategory: ${target.category}\nCommission: ${target.commission}\nApplication URL: ${target.url}`);
    ...
    if (count > 0) await sendEmail({ to: process.env.EMAIL_USER || process.env.SMTP_USER, subject: `[OpsBot] ${count} affiliate applications ready to submit`, html: `<h2>OpsBot Affiliate Report</h2><p>${count} affiliate program applications have been drafted and are ready to submit.</p>`, text: `${count} affiliate applications drafted.` });
...
async function runOutreachOverseer() {
  ...
  try {
    ...
    for (const inquiry of coldInquiries.rows) {
      const followup = await aiChat(`You are an outreach specialist for OzarkRoost, an Ozarks cabin directory.
...
End with a payment link placeholder: [PAYMENT_LINK].`, `Name: ${inquiry.operator_name}\nProperty: ${inquiry.property_name}\nType: ${inquiry.property_type}\nLocation: ${inquiry.location}`);
...
async function sendDailyDigest() { /* unchanged digest behavior is intentionally retained by the existing dashboard/reporting flow */ }
  ...
  return new Promise((resolve) => {
    ...
    imap.once('ready', () => imap.openBox('INBOX', false, (err) => {
      ...
      imap.search(['UNSEEN'], (err, results) => {
        ...
        f.on('message', (msg) => { let raw = ''; msg.on('body', (stream) => stream.on('data', (chunk) => { raw += chunk.toString('utf8'); })); msg.once('end', async () => { try { const parsed = await simpleParser(raw); const sender = parsed.from?.text || parsed.from?.value?.[0]?.address || 'unknown'; const subject = parsed.subject || '(no subject)'; const text = parsed.text || ''; const html = parsed.html || ''; const existing = await pool.query(`SELECT id FROM opsbot_inbound_emails WHERE sender = $1 AND subject = $2 AND received_at > NOW() - INTERVAL '1 hour' LIMIT 1`, [sender, subject]); if (!existing.rows.length) { await pool.query(`INSERT INTO opsbot_inbound_emails (sender, subject, body_text, body_html, status, received_at) VALUES ($1, $2, $3, $4, 'unread', NOW())`, [sender, subject, text, html]); emailsAdded++; console.log(`[OpsBot:IMAP] Queued: ${subject} from ${sender}`); } } catch (e) { console.error('[OpsBot:IMAP] Parse error:', e.message); } }); }); f.once('end', () => imap.end()); f.once('error', (e) => { console.error('[OpsBot:IMAP] Fetch error:', e.message); imap.end(); });
      });
    }));
    ...
    imap.connect(); setTimeout(() => { try { imap.end(); } catch (_) {} resolve(); }, 30000);
  });
}
