...
try {
  ...
  console.warn('[OpsBot] optional dep "imap" missing');
}
try {
  ({ simpleParser } = require('mailparser'));
} catch {
  console.warn('[OpsBot] optional dep "mailparser" missing');
}
...
async function fetchInboundEmails() {
  if (!Imap || !simpleParser) {
    console.log('[OpsBot:IMAP] imap/mailparser not installed — skipping email fetch.');
    return;
  }
