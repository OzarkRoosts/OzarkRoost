...
const OUTREACH_PHYSICAL_ADDRESS = (process.env.OUTREACH_PHYSICAL_ADDRESS || '416 South Oak Street, Perryville, Arkansas').trim();
const REPLYABLE_INBOUND_CATEGORIES = new Set(['listing_inquiry', 'affiliate_application', 'partnership_inquiry', 'support_request']);
...
async function sendEmail({ to, subject, html, text }) {
  ...
  try {
    const safeText = withComplianceFooter(text);
    const safeHtml = html ? `${html}<br><br><strong>Ozark Roosts</strong><br>${OUTREACH_PHYSICAL_ADDRESS.replace(/</g, '&lt;').replace(/>/g, '&gt;')}<br>To stop receiving these messages, reply STOP.` : `<p>${safeText.replace(/\n/g, '<br>')}</p>`;
    const info = await sendOutboundEmail({ from: `OzarkRoost OpsBot <${from}>`, to, subject, html: safeHtml, text: safeText });
...
async function logEmail({ to, subject, status, messageId, error }) {
  try {
    await pool.query(`INSERT INTO opsbot_email_log (recipient, subject, status, message_id, error_message, sent_at) VALUES ($1, $2, $3, $4, $5, NOW())`, [to, subject, status, messageId || null, error || null]);
  } catch (_) {}
}
...
async function runEmailSmokeTest() {
  ...
  if (!to) return;
  try {
    const recent = await pool.query(`SELECT id FROM opsbot_email_log WHERE subject = $1 AND sent_at > NOW() - INTERVAL '24 hours' LIMIT 1`, ['[OpsBot] Outbound email smoke test']);
    if (recent.rows.length) return;
  } catch (_) {}
  const result = await sendEmail({ to, subject: '[OpsBot] Outbound email smoke test', html: '<p>OzarkRoost outbound email is working. This is an automated delivery test from OpsBot.</p>', text: 'OzarkRoost outbound email is working. This is an automated delivery test from OpsBot.' });
...
async function aiChat(systemPrompt, userPrompt, maxTokens = 400) {
  const resp = await AI.chat.completions.create({ model: AI_MODEL, temperature: 0.35, max_tokens: maxTokens, messages: [{ role: 'system', content: systemPrompt }, { role: 'user', content: userPrompt }] });
...
async function processInboundEmail(email) {
  try {
    ...
    if (!REPLYABLE_INBOUND_CATEGORIES.has(category)) {
      await pool.query(`UPDATE opsbot_inbound_emails SET status = 'processed', category = $1 WHERE id = $2`, [category || 'other', email.id]);
      console.log(`[OpsBot:Email] Classified without reply: ${email.subject} (${category || 'other'})`);
      return;
    }

    const suppressed = await pool.query(`SELECT email FROM outreach_suppressions WHERE lower(email) = lower($1) LIMIT 1`, [email.sender]);
    if (suppressed.rows.length) {
      await pool.query(`UPDATE opsbot_inbound_emails SET status = 'processed', category = $1 WHERE id = $2`, [category, email.id]);
      console.log(`[OpsBot:Email] Suppressed sender — no reply: ${email.sender}`);
      return;
    }

    const reply = await aiChat(
      `You are OpsBot, the autonomous operations assistant for OzarkRoost.\nWrite a professional, warm, concise reply (3-5 sentences max).\nOzarkRoost is an Ozarks cabin & outdoor directory. The currently published dashboard offers premium listings at $29, $59, and $99/month. Do not invent discounts, approvals, relationships, or payment status. Always end with a clear next step.`,
    ...
    await pool.query(`UPDATE opsbot_inbound_emails SET status = 'processed', category = $1, replied_at = NOW() WHERE id = $2`, [category, email.id]);
    console.log(`[OpsBot:Email] Processed: ${email.subject} (${category})`);
  } catch (err) {
...
async function runOutreachOverseer() {
  ...
  try {
    ...
    for (const inquiry of coldInquiries.rows) {
      const suppressed = await pool.query(`SELECT email FROM outreach_suppressions WHERE lower(email) = lower($1) LIMIT 1`, [inquiry.email]);
      if (suppressed.rows.length) continue;
      const followup = await aiChat(
        `You are an outreach specialist for OzarkRoost, an Ozarks cabin directory.\nWrite a short, warm follow-up email (3-4 sentences) to a cabin operator who previously inquired about listing.\nThe published dashboard offers premium listings at $29, $59, and $99/month. Do not invent claims. End with a clear next step.`,
      ...
      const text = withComplianceFooter(followup.replace('[PAYMENT_LINK]', paymentLink));
      const html = text.replace(/\n/g, '<br>').replace(paymentLink, `<a href="${paymentLink}">${paymentLink}</a>`);
      const sent = await sendEmail({ to: inquiry.email, subject: `Following up — List ${inquiry.property_name} on OzarkRoost`, html, text });
      if (!sent) continue;
