 ...
 * Only sends to explicit public business contacts seeded in opsbot_sales_prospects.
...
function renderEmail(prospect) {
  ...
  const text = [`Hi ${prospect.business_name} team,`,'',`I'm reaching out because ${prospect.business_name} is exactly the kind of local business travelers look for when planning an Ozarks trip. ${prospect.hook || ''}`,'',`OzarkRoost is building a focused Ozarks travel directory for stays, outdoor adventures, food and local businesses. We're offering a limited number of founding directory spots free while we build the local network.`,'',`We'd be happy to add ${prospect.business_name} with your official details and a direct link. If you want to claim the spot or correct anything, reply to this email and we'll take care of it.`,'',`See the directory: https://ozartkroost.onrender.com`,`Claim/list: https://ozartkroost.onrender.com/list-your-cabin`,'',`If you do not want future messages from OzarkRoost, reply "unsubscribe" and we will stop.`,'',`Thanks,`,`OzarkRoost`].join('\n');
  const html = text.replace(/https:\/\/[^\s]+/g, (url) => `<a href="${url}">${url}</a>`).replace(/\n/g, '<br>');
  return { subject, text, html };
}
async function run() {
  if (!enabled()) { console.log('[OpsBot:ProactiveOutreach] disabled — set OPSBOT_PROACTIVE_OUTREACH=true to enable'); return { sent: 0, skipped: 0, enabled: false }; }
  try {
    const { rows } = await pool.query(`SELECT p.id, p.business_name, p.area, p.email, p.hook, p.source_url FROM opsbot_sales_prospects p WHERE p.opted_out = FALSE AND NOT EXISTS (SELECT 1 FROM opsbot_outreach_log o WHERE o.outreach_type = 'proactive_operator' AND o.reference_id = p.id) ORDER BY p.id ASC LIMIT 3`);
    let sent = 0;
