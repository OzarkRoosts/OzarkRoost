 ...
 * Local, zero-credit, failure-isolated campaign generation.
 * No OpenAI/Groq API calls are made by this module.
 */
...
async function aiGenerate(systemPrompt, userPrompt, maxTokens = 800) {
  // Compatibility wrapper retained so existing marketing jobs keep their API.
  // Generation is entirely local and does not consume provider credits.
  return localAI.generate(systemPrompt, userPrompt, { maxTokens });
}
...
async function generateSEOContent() {
  console.log('[Marketing:SEO] Generating SEO content locally...');
  const pages = ['home', 'listings', 'adventures', 'operators', 'referral'];
  for (const page of pages) {
    const content = await aiGenerate('You are an SEO expert.', `Write 3 meta description variations for the ${page} page of ${SITE_NAME}. Focus on Buffalo River cabins, Ozark adventures and Arkansas travel.`);
...
async function generateBlogPost() {
  ...
  const post = await aiGenerate('You are an Arkansas Ozarks travel writer.', `Write an 800-word article titled "${topic}" for ${SITE_NAME}. Include practical advice, natural booking calls-to-action and keywords Buffalo River cabins, Ozark vacation and Arkansas cabin rental.`);
...
async function generateSocialPosts() {
  for (const platform of ['twitter', 'facebook', 'instagram']) {
    const posts = await aiGenerate(`You are a ${platform} travel social manager.`, `Create 5 ${platform} posts for ${SITE_NAME}. Promote Buffalo River cabins, Ozark adventures and ${SITE_URL}.`);
...
async function runFullCampaign() {
  console.log('[Marketing] Campaign starting — LOCAL AI + failure isolation enabled');
  const jobs = [
...
function start() {
  console.log(`[Marketing] 🚀 ${SITE_NAME} local adaptive marketing engine starting`);
  ...
  console.log('[Marketing] ✅ Armed — daily self-adapting campaigns; no paid AI provider required');
}
