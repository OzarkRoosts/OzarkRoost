...
const SITE_URL = String(process.env.SITE_URL || process.env.APP_URL || process.env.RENDER_EXTERNAL_URL || 'https://ozarkroost.onrender.com').replace(/\/$/, '');
...
async function resolveModel(entry) {
  ...
  try {
    ...
    // Keep the configured model as a last resort; a request-level failure can
    // still trigger provider failover below.
...
async function generateSitemap() {
  const listings = await safeDb('Sitemap', () => pool.query('SELECT id, slug FROM listings WHERE status = $1', ['active']));
  const paths = ['/', '/listings', '/adventures', '/operators', '/referral', '/list-your-cabin', '/faq'];
  for (const row of listings?.rows || []) paths.push(`/listings/${row.slug || row.id}`);
