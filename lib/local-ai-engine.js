 ...
 * Zero-credit, zero-network-by-default text engine for always-on bots.
 * External providers are used only when LOCAL_AI_EMERGENCY=true.
 */
...
const pick = (items, seed = '') => { let n = 0; for (const c of String(seed)) n = (n * 31 + c.charCodeAt(0)) >>> 0; return items[n % items.length]; };
...
function seo(page) {
  ...
  return [`${SITE_NAME}: ${subject} resources, Buffalo River cabins and Ozark adventures. Plan your Arkansas getaway with local travel ideas and stays.`,`Explore ${subject} with ${SITE_NAME} — Buffalo River cabins, outdoor adventures and practical Arkansas Ozarks travel planning in one place.`,`Plan a better Ozarks trip: discover ${subject}, cabin stays and Buffalo River adventures with ${SITE_NAME}.`].join('\n');
}
...
function social(platform) {
  const variants = [`The Ozarks are calling. Find a Buffalo River cabin, pick an adventure and make the weekend count. ${SITE_URL}/listings`,`Cabin + river + fresh air = a better Arkansas getaway. Explore Ozark stays and adventures with ${SITE_NAME}. ${SITE_URL}/adventures`,`Planning the Buffalo River? Start with your base, check conditions, then build the adventure around it. ${SITE_URL}/guides/buffalo-river-cabins`,`Save this for your next Ozarks trip: scenic drives, river time, trails and a cabin that makes coming back easy. ${SITE_URL}/listings`,`Looking for an Arkansas cabin rental? ${SITE_NAME} connects travelers with Ozarks stays and trip ideas. ${SITE_URL}/listings`];
...
function affiliateApplication(prompt) {
  const text = clean(prompt);
  const program = text.match(/Affiliate program:\s*([^\n]+)/i)?.[1] || 'your affiliate program';
  const category = text.match(/Category:\s*([^\n]+)/i)?.[1] || 'travel';
  return `Hello ${program} team,\n\nI’m reaching out on behalf of ${SITE_NAME}, an Arkansas Ozarks travel directory focused on Buffalo River stays, outdoor recreation and practical trip planning. Our audience is specifically interested in ${category} travel products and experiences, making your program a natural fit for our guides and destination content.\n\nWe’d like to apply for your affiliate program and use approved links and disclosures in relevant Ozarks travel content. Please let us know the application requirements and the appropriate next step.\n\nThank you,\nOzarkRoost Partnerships`;
}
...
function reply(prompt) {
  const category = classifyEmail(prompt);
  const lead = category === 'payment_confirmation' ? 'Thanks for the payment update. We’ll verify the transaction and connect it to the appropriate OzarkRoost record.' : category === 'listing_inquiry' ? 'Thanks for reaching out about listing with OzarkRoost. We can help get your property in front of travelers planning Ozarks stays.' : category === 'affiliate_application' ? 'Thanks for your interest in partnering with OzarkRoost. We’ll review the program details and follow the appropriate application process.' : category === 'partnership_inquiry' ? 'Thanks for contacting OzarkRoost about a partnership. We’re interested in practical collaborations that improve the traveler experience and support regional businesses.' : 'Thanks for reaching out to OzarkRoost. We received your message and will review the details.';
...
function generate(systemPrompt, userPrompt) {
  ...
  if (system.includes('affiliate program application')) return affiliateApplication(user);
  if (system.includes('social') || /create 5 .* posts/i.test(user)) return social((system.match(/a (twitter|facebook|instagram)/)?.[1]) || 'social');
  if (system.includes('seo') || /meta description/i.test(user)) return seo(user.match(/for the (.*?) page/i)?.[1] || 'Ozarks travel');
  if (system.includes('travel writer') || /write an 800-word article/i.test(user)) return blog(user.match(/titled\s+"([^"]+)"/i)?.[1] || pick(topics, user));
  if (system.includes('conversion-focused') || /landing-page copy/i.test(user)) return landing(user.match(/targeting\s+"([^"]+)"/i)?.[1] || 'Ozarks vacation rentals');
...
function createEmergencyClient(originalOpenAI) {
  return class LocalFirstOpenAI {
    ...
    get chat() { return { completions: { create: async (request) => { try { const result = generate(request.messages?.[0]?.content || '', request.messages?.[1]?.content || ''); return { choices: [{ message: { content: result } }] }; } catch (localError) { if (process.env.LOCAL_AI_EMERGENCY === 'true' && originalOpenAI) { console.warn(`[LocalAI] local generation failed; emergency provider enabled: ${localError.message}`); const fallback = new originalOpenAI(this.options); return fallback.chat.completions.create(request); } throw localError; } } } }; }
    get models() { return { list: async () => ({ data: [{ id: 'ozarkroost-local-v1', owned_by: 'OzarkRoost' }] }) }; }
  };
}
