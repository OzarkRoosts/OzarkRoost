 ...
 *
 * Usage:
 *   const health = require('./site-health-agent');
 *   health.start();
 *   health.getStatus();
 */
...
function baseUrl() {
  return String(process.env.APP_URL || `http://127.0.0.1:${process.env.PORT || 3000}`).replace(/\/$/, '');
}
...
async function persistIncident(severity, message, meta = {}) {
  try {
    await pool.query(
      `INSERT INTO agent_incidents (agent, severity, message, meta)
       VALUES ($1, $2, $3, $4)`,
    ...
    if (!/relation .* does not exist/i.test(err.message || '')) {
      console.warn('[SiteHealth] incident persist skipped:', err.message);
    }
  }
}
...
function probeUrl(url, timeoutMs = 8000) {
  return new Promise((resolve) => {
    ...
    const req = lib.get(url, { timeout: timeoutMs, headers: { 'User-Agent': 'OzarkSiteHealth/1.0' } }, (res) => {
      res.resume();
      res.on('end', () => {
        if (settled) return;
        settled = true;
    ...
    req.on('timeout', () => {
      ...
      if (settled) return;
      settled = true;
      resolve({ ok: false, status: 0, ms: Date.now() - startedAt, error: 'timeout' });
    });
    req.on('error', (err) => {
      if (settled) return;
      settled = true;
      resolve({ ok: false, status: 0, ms: Date.now() - startedAt, error: err.message });
    });
  });
}
...
function checkGuideTemplates() {
  ...
  const expected = [
    'hot-tub-cabins.ejs',
    'pet-friendly-cabins.ejs',
    'treehouse-rentals.ejs',
    'glamping-ozarks.ejs',
    'luxury-cabins.ejs',
    'ozarks-road-trip.ejs',
    'buffalo-river-cabins.ejs',
    'trip-planner.ejs',
  ];
  for (const file of expected) {
    if (!fs.existsSync(path.join(GUIDE_VIEWS_DIR, file))) missing.push(file);
  }
...
function checkEnv() {
  ...
  if (!process.env.APP_URL && process.env.NODE_ENV === 'production') {
    warnings.push('APP_URL missing in production (canonical/sitemap may be wrong)');
  }
  const affKeys = ['AFF_STAY22_URL', 'AFF_HIPCAMP_URL', 'AFF_VIATOR_URL', 'AFF_REI_URL', 'AFF_GETYOURGUIDE_URL'];
  const missingAff = affKeys.filter((k) => !process.env[k]);
  if (missingAff.length === affKeys.length) {
    warnings.push('No AFF_* tracked URLs set — using default partner links');
  }
...
async function runHeals(report) {
  ...
  // Soft GC hint when memory high (only if exposed)
  if (report.memory?.warn && typeof global.gc === 'function') {
    try {
      global.gc();
      actions.push(pushHeal('gc', `heap was ${report.memory.heap_mb}MB`));
    } catch {
      /* ignore */
    }
  } else if (report.memory?.warn) {
    actions.push(pushHeal('memory-warn', `heap ${report.memory.heap_mb}MB — consider restart or lower load`));
  }

  // DB consecutive fails — clear error buffer to avoid cascade noise
  ...
  // Missing guide templates — cannot auto-create content, log only
  ...
  if (failed.length) {
    await persistIncident('warning', `${failed.length} path probe(s) failed`, {
      paths: failed.map((f) => f.path),
    });
    actions.push(pushHeal('probe-fail', failed.map((f) => f.path).join(', ')));
  }
...
async function runScan() {
  ...
  const [database, probes] = await Promise.all([
    checkDatabase(),
    Promise.all(
      CRITICAL_PATHS.map(async (p) => {
        // Avoid self-DDoS on cold free tier: only probe /health deeply if APP_URL is remote
  ...
  let status = 'healthy';
  if (!database.ok || probeFails > 3 || (errors.rate >= ERROR_RATE_WARN && errors.total_5xx >= 10)) {
    status = 'degraded';
  }
  ...
  const report = {
    ...
    memory,
    errors,
    recent_heals: healActions.slice(0, 10),
  };
  ...
  try {
    await pool.query(
      `INSERT INTO agent_health_snapshots (agent, status, report)
       VALUES ($1, $2, $3)`,
      ['site-health', status, JSON.stringify(report)]
    );
  } catch {
    /* optional table */
  }
  ...
  console.log(
    `[SiteHealth] ${status} — probes ${report.probe_summary.ok}/${report.probe_summary.total}, db=${database.ok ? 'ok' : 'fail'}`
  );
  return report;
}
...
function getStatus() {
  return {
    ...
    last_run: lastRun,
    report: lastReport,
    heal_log: healActions.slice(0, 20),
    error_stats: errorTracker.getStats(),
  };
}
...
function start(options = {}) {
  ...
  if (!envEnabled() && !options.force) {
    console.log('[SiteHealth] disabled (set SITE_HEALTH_ENABLED=true to arm)');
    return getStatus();
  }

  started = true;
  console.log(`[SiteHealth] armed — interval ${PROBE_INTERVAL_MS}ms`);

  // Delay first scan so HTTP server can bind
  setTimeout(() => {
    runScan().catch((err) => console.error('[SiteHealth] scan error:', err.message));
