...
function start(options = {}) {
  ...
  if (process.env.OUTREACH_AUTOMATION_ENABLED !== 'false') {
    try {
      const outreachRunner = require('./aggressive-outreach-runner');
      outreachRunner.start({ intervalMs: Number(process.env.OUTREACH_INTERVAL_MS || 5 * 60 * 1000) });
      console.log('[AggressiveOutreach] executor started from canonical server startup');
    } catch (err) {
      console.error('[AggressiveOutreach] startup failed:', err.message);
    }
  }
