...
let outreach = null;
try {
  outreach = require('./rover-outreach-agent');
} catch (err) {
  console.warn('[PartnerDiscovery] rover outreach module unavailable; discovery will continue without outreach preparation:', err.message);
}
...
async function prepareQueuedOutreach() {
  ...
  for (const prospect of rows.rows) {
    try {
      if (!outreach?.prepareOutreach) continue;
      prepared.push(await outreach.prepareOutreach(prospect));
    }
...
async function start() {
  ...
  if (process.env.PARTNER_AGENT_ENABLED === 'true') {
    try {
      const partnerAgent = require('./partner-agent-runtime');
      partnerAgent.startPartnerAgent();
    } catch (err) {
      console.error('[PartnerDiscovery] revenue agent startup:', err.message);
    }
  }
