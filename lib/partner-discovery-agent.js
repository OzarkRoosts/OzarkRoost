...
// Seeded targets are intentionally high-confidence categories/platforms.
// Rover can expand this queue through research, but must verify every prospect.
...
const CATEGORY_PLAN = [
  'restaurants', 'bars', 'breweries', 'wineries', 'live-music', 'shows', 'festivals',
  'attractions', 'museums', 'caves', 'zipline', 'rafting', 'kayaking', 'fishing',
  'horseback', 'atv', 'hiking', 'camping', 'rv', 'lodging', 'public-lands', 'tourism'
];
  ...
  for (const prospect of SEED_PROSPECTS) {
    ...
    await pool.query(
      `INSERT INTO partner_prospects
        (name, category, partner_type, website_url, application_url, region, fit_score, verification_status, last_verified_at)
       VALUES ($1,$2,$3,$4,$5,$6,$7,'needs_review',NOW())
...
function buildResearchBrief(category) {
  return {
    ...
    verify: [
      ...
      'current partner or affiliate terms',
      'commission or referral economics when published',
      'geographic relevance to the Ozarks',
      'application/contact path',
...
async function runDiscovery() {
  ...
  const rows = await pool.query(
    `SELECT id,name,category,partner_type,website_url,application_url,fit_score,status,verification_status
     FROM partner_prospects ORDER BY fit_score DESC, updated_at DESC LIMIT 100`
  );
  const result = {
    ...
    next_actions: [
      ...
      'Treat federal/state/local public-land agencies as information/partnership prospects, not assumed affiliate merchants.',
  ...
  await pool.query(
    `INSERT INTO partner_discovery_runs (query, results) VALUES ($1,$2)`,
    [JSON.stringify({ categories: CATEGORY_PLAN }), JSON.stringify(result.prospects)]
  );
  return result;
}
...
async function getProspects({ category, status, limit = 50 } = {}) {
  ...
  const result = await pool.query(
    `SELECT * FROM partner_prospects ${where.length ? `WHERE ${where.join(' AND ')}` : ''}
     ORDER BY fit_score DESC, updated_at DESC LIMIT $${params.length}`,
    params
  );
  return result.rows;
}
