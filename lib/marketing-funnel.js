...
function shouldSuppressOutreach(prospect = {}) {
  return Boolean(prospect.opted_out || prospect.replied_at || prospect.bounced_at);
}
...
function buildTrackedUrl(target, partner, source) {
  ...
  if (target.startsWith('/')) {
    const url = new URL(target, 'https://ozarkroost.local');
    if (source) url.searchParams.set('source', source);
    return `${url.pathname}${url.search}`;
  }
  const params = new URLSearchParams({ to: target, partner: partner || 'direct' });
...
function mailchimpConfig() {
  ...
  if (!dc || dc === apiKey) throw new Error('MAILCHIMP_API_KEY must include its data-center suffix');
...
async function syncSubscriberToMailchimp(subscriber) {
  ...
  if (!config) return { configured: false, synced: false };
  ...
  const memberResponse = await fetch(memberUrl, { method: 'PUT', headers, body: JSON.stringify({ email_address: subscriber.email, status_if_new: 'subscribed', merge_fields: {}, tags: ['ozarkroost-subscriber', `source:${subscriber.source}`] }) });
  if (!memberResponse.ok) throw new Error(`Mailchimp member sync failed (${memberResponse.status})`);
  const tagResponse = await fetch(`${memberUrl}/tags`, { method: 'POST', headers, body: JSON.stringify({ tags: [{ name: 'ozarkroost-subscriber', status: 'active' }, { name: 'welcome', status: 'active' }] }) });
  if (!tagResponse.ok) throw new Error(`Mailchimp tag sync failed (${tagResponse.status})`);
