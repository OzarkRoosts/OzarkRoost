...
const SITE = process.env.APP_URL || 'https://ozartkroost.onrender.com';
const FROM = process.env.EMAIL_FROM || process.env.EMAIL_USER;
const transporter = process.env.EMAIL_USER && process.env.EMAIL_PASSWORD
  ? nodemailer.createTransport({ host: process.env.EMAIL_HOST || 'smtp.gmail.com', port: Number(process.env.EMAIL_PORT) || 587, secure: false, auth: { user: process.env.EMAIL_USER, pass: process.env.EMAIL_PASSWORD } })
  : null;
...
async function sendOne(p, followup=false) {
  if (!transporter || !p.email || p.opted_out) return false;
  ...
  try {
    const info = await transporter.sendMail({from: FROM,to:p.email,subject,text:body,headers:{'List-Unsubscribe': `<mailto:${FROM}?subject=unsubscribe>`}});
    ...
    await pool.query(`INSERT INTO opsbot_email_log (recipient,subject,status,message_id) VALUES ($1,$2,'sent',$3)`,[p.email,subject,info.messageId]);
...
async function cycle() {
  await seed();
  if (!transporter) { console.log('[LocalOutreach] SMTP credentials missing; queue seeded, sending disabled.'); return; }
