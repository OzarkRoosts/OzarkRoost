 ...
 *
 * Policy: free affiliate/publisher onboarding may be completed autonomously
 * using credentials supplied through environment variables. Payment, paid
 * plans/subscriptions, CAPTCHA/MFA/human verification, identity attestations,
 * and unknown external form destinations remain blocked.
 */
const pool = require('../db/index');
const { ensureAffiliateExecutionSchema } = require('./affiliate-execution-schema');
const {
  isNoSpendApplication,
  isSafeAgreementField,
  credentialFieldValue,
} = require('./affiliate-autonomy-policy');
...
const DISABLED_PROGRAM_RE = /viator/i;
const KNOWN_HOSTS = new Set([
  'stay22.com','www.stay22.com','app.stay22.com',
  'travelpayouts.com','www.travelpayouts.com','support.travelpayouts.com','passport.travelpayouts.com','app.travelpayouts.com',
  'vrbo.com','www.vrbo.com','booking.com','www.booking.com','alltrails.com','www.alltrails.com','rei.com','www.rei.com',
  ...
  'getyourguide.com','partner.getyourguide.com','affiliate-program.amazon.com','airbnb.com','www.airbnb.com',
  'resortpass.com','www.resortpass.com','expedia.com','www.expedia.com'
]);
...
function parseForms(html) {
  ...
  let match;
  while ((match = formRe.exec(html))) forms.push({ attrs: match[1], body: match[2] });
  return forms;
}
...
function parseInputs(body) {
  ...
  while ((match = re.exec(body))) {
    ...
    if (!name) continue;
    const options = [...inner.matchAll(/<option\b([^>]*)>([\s\S]*?)<\/option>/gi)].map(option => ({
      value: attr(option[1], 'value') || option[2].replace(/<[^>]+>/g, '').trim(),
      text: option[2].replace(/<[^>]+>/g, '').trim(),
    }));
    fields.push({
      tag,
      type,
...
function profileValue(field, application) {
  ...
  const description = process.env.AFFILIATE_APPLICANT_DESCRIPTION ||
    'OzarkRoost is an Arkansas Ozarks travel directory connecting travelers with cabins, outdoor activities, and local stays.';

  const credential = credentialFieldValue(field);
...
function buildPayload(fields, application) {
  ...
  for (const field of fields) {
    if (['submit','button','image','reset','file'].includes(field.type)) continue;

    if (['checkbox','radio'].includes(field.type)) {
      if (isSafeAgreementField(field)) {
        params.append(field.name, field.value || 'on');
        continue;
      }
      if (field.required) missing.push(`${field.name} (required selection/attestation)`);
      if (field.checked && field.value) params.append(field.name, field.value || 'on');
      continue;
    }
    ...
    const value = profileValue(field, application);
    const fallback = field.value || field.options[0]?.value || '';
    const finalValue = value == null ? fallback : value;
    if (field.required && !String(finalValue).trim()) missing.push(field.name);
    if (finalValue !== null && finalValue !== undefined) params.append(field.name, String(finalValue));
  }
...
async function fetchWithTimeout(url, options = {}) {
  ...
  try {
    return await fetch(url, {
      ...
      redirect: 'follow',
      headers: {
        'User-Agent': 'OzarkRoost-AffiliateOps/1.1 (+https://ozartkroost.onrender.com)',
        ...(options.headers || {}),
      },
    });
  } finally {
...
async function mark(id, status, details = {}) {
  await pool.query(
    `UPDATE opsbot_affiliate_applications
     SET status=$1::text,
         applied_at=CASE WHEN $1::text IN ('submitted','applied') THEN NOW() ELSE applied_at END,
         execution_method=$2,
         response_status=$3,
         response_url=$4,
         last_error=$5,
         updated_at=NOW()
     WHERE id=$6`,
    [status, details.method || 'web-form', details.httpStatus || null, details.url || null, details.error || null, id]
  );
}

async function executeApplication(application) {
  if (DISABLED_PROGRAM_RE.test(application.program_name || '')) {
    await mark(application.id, 'disabled', {
      method: 'policy',
      error: 'Viator affiliate execution is intentionally disabled.',
    });
    console.log(`[AffiliateExecutor] disabled: ${application.program_name}`);
    return 'disabled';
  }
  ...
  if (hasBlocker(html)) {
    await mark(application.id, 'needs_human', {
      error: 'CAPTCHA, human verification, MFA, or anti-bot challenge detected.',
  ...
  if (!isNoSpendApplication({ programName: application.program_name, html })) {
    await mark(application.id, 'needs_human', {
      method: 'policy',
      error: 'Potential payment, paid plan, billing, purchase, or subscription language detected; zero-spend policy blocked submission.',
      httpStatus: page.status,
      url: page.url,
    });
    return 'needs_human';
  ...
  if (!forms.length) {
    await mark(application.id, 'needs_human', {
      error: 'No HTML application form found; target likely uses an external portal or JavaScript workflow.',
      httpStatus: page.status,
      url: page.url,
    });
    return 'needs_human';
  ...
  if (missing.length) {
    await mark(application.id, 'needs_human', {
      error: `Required fields could not be safely populated: ${missing.join(', ')}`,
      httpStatus: page.status,
      url: page.url,
    });
    return 'needs_human';
  ...
  if (!action || !hostAllowed(action)) {
    await mark(application.id, 'needs_human', {
      error: 'Form action resolves outside the approved affiliate target allowlist.',
      httpStatus: page.status,
      url: action || page.url,
    });
    return 'needs_human';
  ...
  if (hasBlocker(resultHtml)) {
    await mark(application.id, 'needs_human', {
      error: 'Post-submit human verification detected.',
  ...
  if (!success) {
    await mark(application.id, 'failed', {
      error: `Application submission did not produce a success signal (HTTP ${response.status}).`,
      httpStatus: response.status,
      url: response.url,
    });
    return 'failed';
  ...
  await mark(application.id, 'submitted', {
    method: `web-form:${method}:no-spend`,
    httpStatus: response.status,
    url: response.url,
  });
...
async function runCycle() {
  ...
  await ensureAffiliateExecutionSchema();

  await pool.query(
    `UPDATE opsbot_affiliate_applications
     SET status='disabled', last_error='Viator affiliate execution disabled', updated_at=NOW()
     WHERE program_name ILIKE '%viator%' AND status IN ('drafted','queued','retry','processing')`
  ).catch(() => {});

  await pool.query(
    `UPDATE opsbot_affiliate_applications
     SET status='queued', last_error='Executor restarted before completion', updated_at=NOW()
     WHERE status='processing' AND updated_at < NOW() - INTERVAL '15 minutes'
       AND program_name NOT ILIKE '%viator%'`
  ).catch(() => {});

  const { rows } = await pool.query(
    `SELECT * FROM opsbot_affiliate_applications
     WHERE status IN ('drafted','queued','retry')
       AND program_name NOT ILIKE '%viator%'
     ORDER BY created_at ASC LIMIT $1::integer`,
    [MAX_PER_CYCLE]
  );
  if (!rows.length) return;

  console.log(`[AffiliateExecutor] executing ${rows.length} queued affiliate applications (no-spend mode)`);
  for (const row of rows) {
    try {
      await executeApplication(row);
    } catch (err) {
      await mark(row.id, 'retry', { error: err.message });
      console.error(`[AffiliateExecutor] ${row.program_name}: ${err.message}`);
    }
  }
}
...
function start() {
  if (started) return;
  started = true;
  console.log('[AffiliateExecutor] autonomous application execution armed — no-spend policy active');
...
module.exports = { start, stop, runCycle, executeApplication, parseInputs, buildPayload };
