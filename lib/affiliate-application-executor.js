 ...
 * It never spends money or accepts paid/legal/identity commitments.
...
// Viator is intentionally disabled. Stay22 and Travelpayouts remain active.
const DISABLED_PROGRAM_RE = /viator/i;
const PAYMENT_RE = /credit.?card|debit.?card|card.?number|cvv|cvc|payment|billing|charge|price|pricing|cost|paid\s+(plan|subscription|membership)|subscription|purchase|buy\s+now|checkout/i;
const LEGAL_RE = /terms|conditions|agreement|contract|legal|certif(?:y|ication)|attest|authorize|consent|accept.*(policy|terms|agreement)/i;
const IDENTITY_RE = /social.?security|ssn|tax.?id|taxpayer|ein\b|identity|government.?id|passport|driver.?license|bank.?account|routing.?number/i;
const UNKNOWN_SAFE_FIELD_RE = /^(name|first.?name|last.?name|email|e.?mail|website|url|site|homepage|company|organization|business|publisher|brand|description|about|message|comments|notes|reason|details|promotion|phone|tel|country|traffic|audience|reach)$/i;
...
const KNOWN_HOSTS = new Set([
  ...
  'getyourguide.com','affiliate-program.amazon.com','airbnb.com','www.airbnb.com'
]);
...
function hasPaidCommitment(html) {
  return PAYMENT_RE.test(html);
}
...
/**
 * Classify whether a form can be executed without spending money or making
 * a legal/identity commitment. Unknown required fields fail closed.
 */
function classifyApplicationFields(fields) {
  const reasons = [];
  for (const field of fields) {
    const key = String(field.name || '').trim().toLowerCase();
    if (!field.required) continue;
    if (PAYMENT_RE.test(key) || ['payment','billing'].includes(field.type)) reasons.push(`payment field: ${field.name}`);
    else if (LEGAL_RE.test(key) || field.type === 'checkbox' && /terms|agreement|consent|certif|attest/i.test(key)) reasons.push(`legal acceptance: ${field.name}`);
    else if (IDENTITY_RE.test(key)) reasons.push(`identity/tax field: ${field.name}`);
    else if (!UNKNOWN_SAFE_FIELD_RE.test(key.replace(/[_-]+/g, ' '))) reasons.push(`unknown/unsupported required field: ${field.name}`);
  }
  return { mode: reasons.length ? 'human' : 'auto', reasons };
}
...
async function mark(id, status, details = {}) {
  await pool.query(`UPDATE opsbot_affiliate_applications SET status=$1::text, applied_at=CASE WHEN $1::text IN ('submitted','applied') THEN NOW() ELSE applied_at END, execution_method=$2, response_status=$3, response_url=$4, last_error=$5, updated_at=NOW() WHERE id=$6`, [status, details.method || 'web-form', details.httpStatus || null, details.url || null, details.error || null, id]);
}
async function executeApplication(application) {
  if (DISABLED_PROGRAM_RE.test(application.program_name || '')) {
    await mark(application.id, 'disabled', { method: 'policy', error: 'Viator affiliate execution is intentionally disabled; Stay22 and Travelpayouts remain active.' });
  ...
  if (hasPaidCommitment(html)) {
    await mark(application.id, 'needs_human', { error: 'Paid commitment/payment language detected; zero-spend policy requires human review.', httpStatus: page.status, url: page.url });
    return 'needs_human';
  }
  ...
  const policy = classifyApplicationFields(fields);
  if (policy.mode === 'human') {
    await mark(application.id, 'needs_human', { error: `Zero-spend policy requires human review: ${policy.reasons.join('; ')}`, httpStatus: page.status, url: page.url });
    return 'needs_human';
  }
...
async function runCycle() {
  ...
  // Permanently retire any queued/retry Viator applications so they cannot be executed again.
  await pool.query(`UPDATE opsbot_affiliate_applications SET status='disabled', last_error='Viator affiliate execution disabled', updated_at=NOW() WHERE program_name ILIKE '%viator%' AND status IN ('drafted','queued','retry','processing')`).catch(() => {});
  await pool.query(`UPDATE opsbot_affiliate_applications SET status='queued', last_error='Executor restarted before completion', updated_at=NOW() WHERE status='processing' AND updated_at < NOW() - INTERVAL '15 minutes' AND program_name NOT ILIKE '%viator%'`).catch(() => {});
  const { rows } = await pool.query(`SELECT * FROM opsbot_affiliate_applications WHERE status IN ('drafted','queued','retry') AND program_name NOT ILIKE '%viator%' ORDER BY created_at ASC LIMIT $1::integer`, [MAX_PER_CYCLE]);
...
module.exports = { start, stop, runCycle, executeApplication, classifyApplicationFields };
