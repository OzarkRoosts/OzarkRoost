...
async function fetchWithTimeout(url, options = {}) {
  ...
  try { return await fetch(url, { ...options, signal: controller.signal, redirect: 'follow', headers: { 'User-Agent': 'OzarkRoost-AffiliateOps/1.0 (+https://ozarkroost.onrender.com)', ...(options.headers || {}) } }); }
  finally { clearTimeout(timer); }
}

async function mark(id, status, details = {}) {
  await pool.query(`UPDATE opsbot_affiliate_applications SET status=$1, applied_at=CASE WHEN $1='submitted' THEN NOW() ELSE applied_at END, execution_method=$2, response_status=$3, response_url=$4, last_error=$5, updated_at=NOW() WHERE id=$6`, [status, details.method || 'web-form', details.httpStatus || null, details.url || null, details.error || null, id]);
}
...
async function executeApplication(application) {
  ...
  let response;
  if (method === 'post') {
    response = await fetchWithTimeout(action, { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded' }, body: params.toString() });
  } else {
...
async function runCycle() {
  ...
  const { rows } = await pool.query(`SELECT * FROM opsbot_affiliate_applications WHERE status IN ('drafted','queued','retry') ORDER BY priority DESC NULLS LAST, created_at ASC LIMIT $1`, [MAX_PER_CYCLE]);
